<!--
> **部分取代横幅(2026-08-28 深夜后)**:决策 1(GuardedModule 运行时护栏)
> 与修订 1/2/3 的执行面部分(枚举写出器/能力注入/import 白名单)已被
> ADR-0009(出域单点)取代——用户终裁"任何一层都不限制代码执行面,唯一
> 红线=行值出域"。决策 2(Excel 公式中和)、决策 3(worker 环境白名单)
> 与修订 4/5 仍然有效。该形态已入库(commit 61dc8a9),可随时 revert。
-->
