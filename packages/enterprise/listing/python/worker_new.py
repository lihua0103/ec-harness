...
from styles import create_multi_sheet_excel, merge_listing_files
...
def operation_publish(request: dict) -> dict:
    """
    publish 操作：生成多 Sheet Excel 输出（新版）
    """
    ...
    try:
        ...
        # 使用新的多 sheet 写入器
        result = create_multi_sheet_excel(
            outputs=outputs,
...
def operation_merge_listings(request: dict) -> dict:
    """
    merge_listings 操作：合并现有的单独 listing 文件
    用于迁移旧的单文件输出模式
    """
    project = request["project"]
    scenario = request.get("scenario", "manual")
    source_dir = request.get("sourceDir")
    
    project_dir = Path(project).resolve()
    
    if source_dir:
        listing_dir = Path(source_dir).resolve()
    else:
        listing_dir = project_dir / ".clinical-listing" / "output" / scenario
    
    if not listing_dir.exists():
        return {
            "ok": False,
            "code": "SOURCE_DIR_NOT_FOUND",
            "reason": f"源目录不存在: {listing_dir}",
        }
    
    output_dir = project_dir / ".clinical-listing" / "output" / scenario
    output_dir.mkdir(parents=True, exist_ok=True)
    output_file = output_dir / f"{scenario.upper()}_LISTINGS_MERGED.xlsx"
    
    try:
        result = merge_listing_files(
...
def main():
    ...
    try:
        ...
        elif operation == "listing_merge":
            response = operation_merge_listings(request)
        else:
            response = {
                "ok": False,
