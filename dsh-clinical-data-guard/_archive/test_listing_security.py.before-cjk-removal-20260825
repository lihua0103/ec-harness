...
def test_spec_profile_normalizes_cjk_digits_before_release() -> None:
    """spec 允许保留需求语义，但中文数字不能成为日期和编号的编码绕过。"""
    source = "方案日期为二零二六年八月十九日，受试者编号一零一零零一。"
    scrubbed, stats = smart_scrub_text(source, profile="spec")
    assert source not in scrubbed
    assert "二零二六年八月十九日" not in scrubbed
    assert "一零一零零一" not in scrubbed
